...
`".agents/skills/seo/scripts/claude-seo" run <script.py>`. That is the single
...
`".agents/skills/seo/scripts/claude-seo" setup`, report core and Chromium
status separately, and do not fall back to global or user package installation.
For diagnosis, execute
`".agents/skills/seo/scripts/claude-seo" doctor --json`; its output
intentionally omits absolute paths and environment values. If any
`".agents/skills/seo/scripts/claude-seo" run` command reports that setup is
...
3. If Google API credentials detected (`".agents/skills/seo/scripts/claude-seo" run google_auth.py --check`), also spawn seo-google agent
...
6. If backlink APIs detected (`".agents/skills/seo/scripts/claude-seo" run backlinks_auth.py --check`), also spawn seo-backlinks agent
...
10. If drift baseline exists for this URL (`".agents/skills/seo/scripts/claude-seo" run drift_history.py <url>`), also spawn seo-drift agent
...
After any analysis command completes, offer to generate a PDF report via `".agents/skills/seo/scripts/claude-seo" run google_report.py`.
