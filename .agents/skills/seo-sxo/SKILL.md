...
1. Fetch the target URL via `".agents/skills/seo/scripts/claude-seo" run render_page.py <URL> --mode auto` (SPA-aware and SSRF-safe)
2. Parse with `".agents/skills/seo/scripts/claude-seo" run parse_html.py <URL>` to extract: title, H1, meta description,
...
- [ ] Target URL was fetched via `".agents/skills/seo/scripts/claude-seo" run render_page.py <URL> --mode auto` (not raw curl/fetch)
