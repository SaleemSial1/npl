...
- XML sitemap: run `".agents/skills/seo/scripts/claude-seo" run sitemap_discovery.py <url> --json`; require a
...
".agents/skills/seo/scripts/claude-seo" run agent_ux_check.py https://example.com --json
```
...
`".agents/skills/seo/scripts/claude-seo" run render_page.py <url> --a11y-tree --json`.
...
If Google API credentials are configured, use `".agents/skills/seo/scripts/claude-seo" run pagespeed_check.py <url> --json` for real PSI + CrUX field data (replaces lab-only CWV estimates), `".agents/skills/seo/scripts/claude-seo" run crux_history.py <url> --json` for 25-week CWV trends, and `".agents/skills/seo/scripts/claude-seo" run gsc_inspect.py <url> --json` for real indexation status per URL.
...
CLAUDE_SEO_LOCAL_TARGETS="localhost:3000,127.0.0.1:8080,100.101.102.103" \
  ".agents/skills/seo/scripts/claude-seo" run fetch_page.py http://localhost:3000/
```
