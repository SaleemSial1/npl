...
Read `AGENTS.md` before working on news. `data/news.json` is the news source of truth. `npm run news:generate` updates article pages, archive, homepage cards, and sitemaps. `npm run search:generate` rebuilds the title-based search index after page changes. `npm run build` runs both.

`npm run news:fetch` only extracts research to `.news-cache`; it does not publish. `npm run auction:sync` fetches the auction sources and rewrites the auction snapshot. Missing source totals stop the sync instead of publishing inferred numbers. Historical timeline entries do not establish that an auction is currently live.
