...
function updateHomepageNewsSection(items) {
  ...
  const cards = items.slice(0, 8).map(renderHomepageCard).join('\n\n');
...
function updateHomepageHeroNewsCards(items) {
  ...
  const cards = items.slice(0, 8).map(renderHeroNewsCard).join('\n');
  const replacement = `${startMarker}
                            <!-- Generated by scripts/generate-news-pages.js from data/news.json -->
${cards}`;
  fs.writeFileSync(homepagePath, `${html.slice(0, start)}${replacement}${html.slice(end)}`);
...
function injectInternalLinks(bodyParagraphs) {
  ...
  return bodyParagraphs.map((rawParagraph, index) => {
    ...
    return `<p${index === 0 ? ' class="news-article__lead"' : ''}>${pText}</p>`;
  }).join('\n      ');
}
