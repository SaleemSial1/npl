...
const seasonStylesHref = '/season.css?v=20260919-sports';
...
const mainPages = new Set(['teams.html','players.html','all-players.html','international-stars.html','stats.html','auction.html','schedule.html','matches.html','streaming.html','tickets.html','fan-guide.html','venues.html','partners.html','about.html','videos.html','points-table.html']);
...
function shell(file,title,description,body,{noindex=false,entity=null,canonical=null}={}) {
 if (mainOnly && !mainPages.has(file)) return;
 if (mainPages.has(file)) body = body.replaceAll('Records describe the dated reports linked here; a signing does not guarantee selection or availability for every match.', 'The links below identify the dated reports used for this guide. Later fixture notices, scorecards and provider offers may change the relevant details.');
...
function generate() {
 for (const t of mainOnly ? [] : season.teams) {
 ...
 for (const p of mainOnly ? [] : season.players) {
 ...
 shell('teams.html','NPL 2026 Teams & Squad Tracker','Explore all eight NPL teams, source-linked squad records and the distinction between retentions, auction listings and overseas signings.',hero('NPL 2026 Teams','Eight franchises. Source-linked rosters, with disputed and unverified details clearly labelled.')+`<div class="season-grid">${cards}</div>`+section('Compare recruitment, not just squad size','<p>Retained players carried over from an earlier season. Auction listings record a reported domestic transaction, while overseas signings are tracked separately. The number on each card is the number of records in this directory, not an official squad-registration total.</p><p>Open a team to see the source attached to each listing. For a cross-team comparison, use the <a href="/all-players">complete roster table</a>; for prices and disputed outcomes, use the <a href="/auction">auction records</a>.</p>')+note(season.notes[2])+sources(['retentions','pokhara-retentions','auction','auction-report','dates']));
 ...
 shell('players.html','NPL 2026 Players | Search Team Squads','Search the NPL Season 3 roster by player, team and recruitment status. Includes source-linked records and flagged auction conflicts.',hero('NPL 2026 Player Directory','Search retained players, reported auction buys and verified overseas signings. Counts reflect this reviewed directory, not an official final registration list.')+section('How to use this directory','<p>Search a player name, then narrow the list by team or recruitment status. Filters apply together: clear the name or select All teams if a record disappears. A role marked unspecified means the cited roster source does not establish it.</p><p>A listed auction price belongs to that transaction. It is not a retained-player salary or an overseas contract fee. Entries marked as conflicting need confirmation before being treated as completed transfers.</p>')+filter+`<div class="season-grid">${playerCards}</div><p id="rosterEmpty" hidden>No players match these filters.</p>`+section('Roster coverage',`<p>${esc(season.notes[0])}</p><p>${esc(season.notes[2])}</p><p>${link('/all-players','Compare rosters in one table')}</p>`));
 ...
 const stats=hero('NPL 2026 Statistics','Season 3 has not started as of 18 September 2026. Historical figures must not be labelled as 2026 results.')+section('Batting and bowling',table(['Metric','Season 3 status'],[['Runs','Not available before the first match'],['Wickets','Not available before the first match'],['Batting average / strike rate','Not available'],['Bowling average / economy','Not available'],['Leading run-scorer / wicket-taker','Not established']],'2026 match statistics'))+section('What the main cricket statistics measure',table(['Statistic','How to read it'],[['Runs and wickets','Batting and bowling totals for the stated competition and season.'],['Batting average','Runs divided by dismissals. An innings played is not necessarily a dismissal.'],['Strike rate','Runs per 100 balls faced.'],['Bowling average','Runs conceded divided by wickets credited to the bowler.'],['Economy rate','Runs conceded per over bowled. Use legal balls when converting partial overs.']],'Cricket statistics explained')+'<p>Do not combine international figures with an NPL season total. A batting average without a dismissal, or a bowling average without a wicket, is undefined rather than zero. Scorecard overs are ball notation: 3.2 overs means three overs and two legal balls, not 3.2 decimal overs.</p>')+section('How records will be published','<p>Statistics need a season, format, scorecard source and an update date. Squad membership alone does not count as a match played. Missing averages and rates stay unavailable rather than being displayed as zero.</p><p><a href="/points-table">Pre-season points table</a> · <a href="/players">Player directory</a></p>')+sources(['dates']);
 ...
 generateGuides();
 generateAuction();
 console.log(mainOnly ? 'Generated main season pages' : `Generated ${season.players.length} player profiles, 8 team pages and season guides`);
}
...
function generateGuides() {
 ...
 for(const g of guides) {
  if (mainOnly && !mainPages.has(g.file)) continue;
